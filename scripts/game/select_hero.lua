...
local function select_unit (u)
    local trg = cj.CreateTrigger()
    cj.TriggerRegisterUnitEvent(trg, u, cj.EVENT_UNIT_SPELL_CAST)
    -- cj.TriggerRegisterAnyUnitEventBJ(trg,cj.EVENT_PLAYER_UNIT_SPELL_CAST)
    cj.TriggerAddAction(trg,function ()
        ...
        if abi_id == str2id('A005:ANcl') then
            echo("釋放了技能..选择步兵")
            cj.CreateUnit(p,str2id('H003:Hpal'),x,y,0)
        end
        if abi_id == str2id('A007:ANcl') then
            echo("釋放了技能..选择弓箭手")
            cj.CreateUnit(p,str2id('H004:Hpal'),x,y,0)
        end
        if abi_id == str2id('A006:ANcl') then
            echo("釋放了技能..选择火枪手")
            cj.CreateUnit(p,str2id('H002:Hpal'),x,y,0)
        end
        if abi_id == str2id('A003:ANcl') then
            echo("釋放了技能..选择牛头人")
            cj.CreateUnit(p,str2id('H000:Hpal'),x,y,0)
        end
